cd Frontend
1. npm i
2. npm start


cd Backend
1. npm i
2. create new database schema in sql workbench and edit details in db.js
3. npm start (Automatically tables are created and some testing data is also inserted into them)


NOTE: for user you can also create a account of yourself in signup page but for admin the role need to be changed manually in the sql table.

ADMIN : bayyaramharsha@gmail.com
ADMIN PASSWORD : bayyaramharsha

USER : Manjula@gmail.com
USER PASSWORD : Manjula12345